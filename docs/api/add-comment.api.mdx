---
id: add-comment
title: "Add comment"
description: "Add a Comment, optionally by an Id"
sidebar_label: "Add comment"
hide_title: true
hide_table_of_contents: true
api:
  {
    "security": [{ "APITokenAuth": [] }],
    "description": "Add a Comment, optionally by an Id",
    "parameters":
      [
        {
          "in": "path",
          "name": "docId",
          "schema": { "type": "string" },
          "required": true,
          "description": "String Id of the Comment",
        },
      ],
    "requestBody":
      {
        "required": true,
        "content":
          {
            "application/json":
              {
                "schema":
                  {
                    "type": "object",
                    "properties":
                      {
                        "time":
                          {
                            "description": "Milliseconds since epoch for this timestamp/date",
                            "type": "integer",
                          },
                        "text": { "description": "Comment text/message", "type": "string" },
                        "supporMarkdown":
                          {
                            "description": "Whether this comment supports markdown",
                            "type": "boolean",
                          },
                        "documentId":
                          {
                            "description": "Id of the object this comment belongs to",
                            "type": "string",
                          },
                        "collection":
                          {
                            "description": 'Id of the collection this comments belongs to (Only accepted collection is "frontHistory")',
                            "type": "string",
                          },
                      },
                    "title": "commentsPost",
                  },
              },
          },
      },
    "tags": ["Comments"],
    "responses": { "200": { "description": "OK" }, "404": { "description": "NOT FOUND" } },
    "method": "post",
    "path": "/v1/comment/{docId}",
    "servers":
      [{ "url": "https://api.apparyllis.com" }, { "url": "https://devapi.apparyllis.com" }],
    "securitySchemes":
      { "APITokenAuth": { "type": "apiKey", "in": "header", "name": "Authorization" } },
    "jsonRequestBodyExample":
      {
        "time": 0,
        "text": "string",
        "supporMarkdown": true,
        "documentId": "string",
        "collection": "string",
      },
    "info":
      {
        "title": "Simply Plural API",
        "description": "This API reference documents the public api endpoints. You can see a list of all public endpoints here https://github.com/ApparyllisOrg/SimplyPluralApi/blob/main/src/api/v1/routes.ts",
        "version": "1.0",
      },
    "postman":
      {
        "name": "Add comment",
        "description": { "content": "Add a Comment, optionally by an Id", "type": "text/plain" },
        "url":
          {
            "path": ["v1", "comment", ":docId"],
            "host": ["{{baseUrl}}"],
            "query": [],
            "variable":
              [
                {
                  "disabled": false,
                  "description":
                    { "content": "(Required) String Id of the Comment", "type": "text/plain" },
                  "type": "any",
                  "value": "",
                  "key": "docId",
                },
              ],
          },
        "header": [{ "key": "Content-Type", "value": "application/json" }],
        "method": "POST",
        "body": { "mode": "raw", "raw": '""', "options": { "raw": { "language": "json" } } },
        "auth":
          {
            "type": "apikey",
            "apikey":
              [
                { "type": "any", "value": "Authorization", "key": "key" },
                { "type": "any", "value": "<API Key>", "key": "value" },
                { "type": "any", "value": "header", "key": "in" },
              ],
          },
      },
  }
sidebar_class_name: "post api-method"
info_path: docs/api/simply-plural-api
custom_edit_url: null
---

import ApiTabs from "@theme/ApiTabs";
import DiscriminatorTabs from "@theme/DiscriminatorTabs";
import MethodEndpoint from "@theme/ApiDemoPanel/MethodEndpoint";
import MimeTabs from "@theme/MimeTabs";
import ParamsItem from "@theme/ParamsItem";
import ResponseSamples from "@theme/ResponseSamples";
import SchemaItem from "@theme/SchemaItem";
import SchemaTabs from "@theme/SchemaTabs";
import TabItem from "@theme/TabItem";

<h1 className={"openapi__heading"}>Add comment</h1>

<MethodEndpoint method={"post"} path={"/v1/comment/{docId}"}></MethodEndpoint>

Add a Comment, optionally by an Id

<details
  style={{ marginBottom: "1rem" }}
  className={"openapi-markdown__details"}
  data-collapsed={false}
  open={true}>
  <summary style={{}}>
    <strong>Path Parameters</strong>
  </summary>
  <div>
    <ul>
      <ParamsItem
        className={"paramsItem"}
        param={{
          in: "path",
          name: "docId",
          schema: { type: "string" },
          required: true,
          description: "String Id of the Comment",
        }}></ParamsItem>
    </ul>
  </div>
</details>
<MimeTabs className={"openapi-tabs__mime"}>
  <TabItem label={"application/json"} value={"application/json-schema"}>
    <details style={{}} className={"openapi-markdown__details"} data-collapsed={false} open={true}>
      <summary style={{ textAlign: "left" }}>
        <strong>Request Body</strong>
        <strong style={{ fontSize: "var(--ifm-code-font-size)", color: "var(--openapi-required)" }}>
          {" "}
          required
        </strong>
      </summary>
      <div style={{ textAlign: "left", marginLeft: "1rem" }}></div>
      <ul style={{ marginLeft: "1rem" }}>
        <SchemaItem
          collapsible={false}
          name={"time"}
          required={false}
          schemaName={"integer"}
          qualifierMessage={undefined}
          schema={{
            description: "Milliseconds since epoch for this timestamp/date",
            type: "integer",
          }}></SchemaItem>
        <SchemaItem
          collapsible={false}
          name={"text"}
          required={false}
          schemaName={"string"}
          qualifierMessage={undefined}
          schema={{ description: "Comment text/message", type: "string" }}></SchemaItem>
        <SchemaItem
          collapsible={false}
          name={"supporMarkdown"}
          required={false}
          schemaName={"boolean"}
          qualifierMessage={undefined}
          schema={{
            description: "Whether this comment supports markdown",
            type: "boolean",
          }}></SchemaItem>
        <SchemaItem
          collapsible={false}
          name={"documentId"}
          required={false}
          schemaName={"string"}
          qualifierMessage={undefined}
          schema={{
            description: "Id of the object this comment belongs to",
            type: "string",
          }}></SchemaItem>
        <SchemaItem
          collapsible={false}
          name={"collection"}
          required={false}
          schemaName={"string"}
          qualifierMessage={undefined}
          schema={{
            description:
              'Id of the collection this comments belongs to (Only accepted collection is "frontHistory")',
            type: "string",
          }}></SchemaItem>
      </ul>
    </details>
  </TabItem>
</MimeTabs>
<div>
  <div>
    <ApiTabs>
      <TabItem label={"200"} value={"200"}>
        <div>OK</div>
        <div></div>
      </TabItem>
      <TabItem label={"404"} value={"404"}>
        <div>NOT FOUND</div>
        <div></div>
      </TabItem>
    </ApiTabs>
  </div>
</div>
