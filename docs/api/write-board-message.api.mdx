---
id: write-board-message
title: "Write board message"
description: "Write a new board message for a member"
sidebar_label: "Write board message"
hide_title: true
hide_table_of_contents: true
api:
  {
    "security": [{ "APITokenAuth": [] }],
    "description": "Write a new board message for a member",
    "parameters":
      [
        {
          "in": "path",
          "name": "id",
          "schema":
            {
              "schema": { "type": "string" },
              "name": "objectId",
              "in": "path",
              "required": true,
              "description": "Object ID of the object, this can be a mongo ID but also a plain string (backwards-compatibility). When using POST or add API calls, this property is optional.",
              "title": "objectId",
            },
          "required": true,
          "description": "Object ID of the object, this can be a mongo ID but also a plain string (backwards-compatibility). When using POST or add API calls, this property is optional.",
        },
      ],
    "requestBody":
      {
        "required": true,
        "content":
          {
            "application/json":
              {
                "schema":
                  {
                    "type": "object",
                    "properties":
                      {
                        "title": { "description": "Title of the message", "type": "string" },
                        "message":
                          { "description": "The message of the message", "type": "string" },
                        "writtenBy":
                          {
                            "description": "ID of the member that wrote this message",
                            "type": "string",
                          },
                        "writtenFor":
                          {
                            "description": "ID of the member that this message is for",
                            "type": "string",
                          },
                        "writtenAt":
                          {
                            "description": "Milliseconds since epoch for this timestamp/date",
                            "type": "integer",
                          },
                        "read":
                          {
                            "description": "Whether this message is read or not",
                            "type": "boolean",
                          },
                        "supportMarkdown":
                          {
                            "description": "Whether this board message supports markdown",
                            "type": "boolean",
                          },
                      },
                    "title": "boardMessagePost",
                  },
              },
          },
      },
    "tags": ["Board Messages"],
    "responses": { "200": { "description": "OK" }, "404": { "description": "NOT FOUND" } },
    "method": "post",
    "path": "/v1/board/:id",
    "servers":
      [{ "url": "https://api.apparyllis.com" }, { "url": "https://devapi.apparyllis.com" }],
    "securitySchemes":
      { "APITokenAuth": { "type": "apiKey", "in": "header", "name": "Authorization" } },
    "jsonRequestBodyExample":
      {
        "title": "string",
        "message": "string",
        "writtenBy": "string",
        "writtenFor": "string",
        "writtenAt": 0,
        "read": true,
        "supportMarkdown": true,
      },
    "info":
      {
        "title": "Simply Plural API",
        "description": "This API reference documents the public API endpoints. You can see a list of all public endpoints here https://github.com/ApparyllisOrg/SimplyPluralApi/blob/main/src/api/v1/routes.ts",
        "version": "1.0",
      },
  }
sidebar_class_name: "post api-method"
info_path: docs/api/simply-plural-api
custom_edit_url: null
---

import ApiTabs from "@theme/ApiTabs";
import DiscriminatorTabs from "@theme/DiscriminatorTabs";
import MethodEndpoint from "@theme/ApiDemoPanel/MethodEndpoint";
import MimeTabs from "@theme/MimeTabs";
import ParamsItem from "@theme/ParamsItem";
import ResponseSamples from "@theme/ResponseSamples";
import SchemaItem from "@theme/SchemaItem";
import SchemaTabs from "@theme/SchemaTabs";
import TabItem from "@theme/TabItem";

<h1 className={"openapi__heading"}>Write board message</h1>

<MethodEndpoint method={"post"} path={"/v1/board/:id"}></MethodEndpoint>

Write a new board message for a member

<details
  style={{ marginBottom: "1rem" }}
  className={"openapi-markdown__details"}
  data-collapsed={false}
  open={true}>
  <summary style={{}}>
    <strong>Path Parameters</strong>
  </summary>
  <div>
    <ul>
      <ParamsItem
        className={"paramsItem"}
        param={{
          in: "path",
          name: "id",
          schema: {
            schema: { type: "string" },
            name: "objectId",
            in: "path",
            required: true,
            description:
              "Object ID of the object, this can be a mongo ID but also a plain string (backwards-compatibility). When using POST or add API calls, this property is optional.",
            title: "objectId",
          },
          required: true,
          description:
            "Object ID of the object, this can be a mongo ID but also a plain string (backwards-compatibility). When using POST or add API calls, this property is optional.",
        }}></ParamsItem>
    </ul>
  </div>
</details>
<MimeTabs className={"openapi-tabs__mime"}>
  <TabItem label={"application/json"} value={"application/json-schema"}>
    <details style={{}} className={"openapi-markdown__details"} data-collapsed={false} open={true}>
      <summary style={{ textAlign: "left" }}>
        <strong>Request Body</strong>
        <strong style={{ fontSize: "var(--ifm-code-font-size)", color: "var(--openapi-required)" }}>
          {" "}
          required
        </strong>
      </summary>
      <div style={{ textAlign: "left", marginLeft: "1rem" }}></div>
      <ul style={{ marginLeft: "1rem" }}>
        <SchemaItem
          collapsible={false}
          name={"title"}
          required={false}
          schemaName={"string"}
          qualifierMessage={undefined}
          schema={{ description: "Title of the message", type: "string" }}></SchemaItem>
        <SchemaItem
          collapsible={false}
          name={"message"}
          required={false}
          schemaName={"string"}
          qualifierMessage={undefined}
          schema={{ description: "The message of the message", type: "string" }}></SchemaItem>
        <SchemaItem
          collapsible={false}
          name={"writtenBy"}
          required={false}
          schemaName={"string"}
          qualifierMessage={undefined}
          schema={{
            description: "ID of the member that wrote this message",
            type: "string",
          }}></SchemaItem>
        <SchemaItem
          collapsible={false}
          name={"writtenFor"}
          required={false}
          schemaName={"string"}
          qualifierMessage={undefined}
          schema={{
            description: "ID of the member that this message is for",
            type: "string",
          }}></SchemaItem>
        <SchemaItem
          collapsible={false}
          name={"writtenAt"}
          required={false}
          schemaName={"integer"}
          qualifierMessage={undefined}
          schema={{
            description: "Milliseconds since epoch for this timestamp/date",
            type: "integer",
          }}></SchemaItem>
        <SchemaItem
          collapsible={false}
          name={"read"}
          required={false}
          schemaName={"boolean"}
          qualifierMessage={undefined}
          schema={{
            description: "Whether this message is read or not",
            type: "boolean",
          }}></SchemaItem>
        <SchemaItem
          collapsible={false}
          name={"supportMarkdown"}
          required={false}
          schemaName={"boolean"}
          qualifierMessage={undefined}
          schema={{
            description: "Whether this board message supports markdown",
            type: "boolean",
          }}></SchemaItem>
      </ul>
    </details>
  </TabItem>
</MimeTabs>
<div>
  <div>
    <ApiTabs>
      <TabItem label={"200"} value={"200"}>
        <div>OK</div>
        <div></div>
      </TabItem>
      <TabItem label={"404"} value={"404"}>
        <div>NOT FOUND</div>
        <div></div>
      </TabItem>
    </ApiTabs>
  </div>
</div>
